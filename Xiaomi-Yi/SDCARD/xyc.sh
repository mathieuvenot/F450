...
# 
# Update by Tanckom to use with ash.booter (latest version)
# and in connection with XIMODE scripts.
#
...
writeAutoexec ()
{
  ...
  echo "# XIMODE - XIAOMI YI VIDEO & PHOTO BUNDLE" > $OUTFILE
  echo "#         by xs-Sol - xs-sol.com  " > $OUTFILE
  echo "#           contact@xs-sol.com  " > $OUTFILE
  echo "########################################### " >> $OUTFILE
  echo "# Generated by XYC $VERS, `date`" > $OUTFILE
  echo "# https://github.com/alex-agency/XYC" >> $OUTFILE
  echo "########################################### " >> $OUTFILE
  echo "# XIAOMI YI AUTOEXEC.ASH GENERATOR" >> $OUTFILE
  echo "# ASH.BOOTER v0.3 (2015-12-18)" >> $OUTFILE
  echo "#" >> $OUTFILE
  echo "# by Halvaborsch <dsequence@gmail.com> " >> $OUTFILE
  echo "# https://github.com/halvaborsch/ash.booter" >> $OUTFILE
  echo "#" >> $OUTFILE
  echo "###########################################" >> $OUTFILE
  echo "# DO NOT REMOVE! BOOTER SHOULD BE FIRST" >> $OUTFILE
  echo "sleep 3" >> $OUTFILE
  echo "lu_util exec '/tmp/fuse_d/SCRIPTS/booter.sh'" >> $OUTFILE
  echo "# DO NOT REMOVE! BOOTER SHOULD BE FIRST " >> $OUTFILE
  echo "#" >> $OUTFILE
  echo "#" >> $OUTFILE
  echo "#" >> $OUTFILE
  echo "###########################################" >> $OUTFILE
  echo "## XIMODE CUSTOM SCRIPT WITH TELNET ##" >> $OUTFILE
  echo "###########################################" >> $OUTFILE
  echo "" >> $OUTFILE
  echo "# " >> $OUTFILE
  echo "# " >> $OUTFILE
